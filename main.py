...
class CUDACallback(Callback):
    ...
    def on_train_epoch_start(self, trainer, pl_module):
        ...
        gpu_idx = trainer.strategy.root_device.index
    ...
    def on_train_epoch_end(self, trainer, pl_module):
        gpu_idx = trainer.strategy.root_device.index
        torch.cuda.synchronize(gpu_idx)
        max_memory = torch.cuda.max_memory_allocated(gpu_idx) / 2**20
...
if __name__ == "__main__":
    ...
    try:
        ...
        # migrate PL 1.4 gpus= to PL 1.9 accelerator/strategy/devices
        if "gpus" in trainer_config:
            gpuinfo = trainer_config.pop("gpus")
            # parse GPU list: "0," -> ["0"], "0,1," -> ["0","1"]
            gpu_list = [g for g in str(gpuinfo).strip(",").split(",") if g]
            trainer_config["accelerator"] = "gpu"
            trainer_config["devices"] = (
                len(gpu_list) if len(gpu_list) > 1 else [int(gpu_list[0])]
            )
            if len(gpu_list) > 1:
        ...
        default_logger_cfgs = {
            "wandb": {
                ...
                "params": {
                    ...
                    "offline": opt.debug,
                    "id": nowname,
                    "project": os.environ.get("WANDB_PROJECT", "latent-diffusion"),
                },
            },
            "tensorboard": {
        ...
        if opt.debug and "trainer" in dir() and trainer.global_rank == 0:
            try:
